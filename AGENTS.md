...
3. **Explain the "why", not just the "what".** Why `privileged`, why userspace datapath, why `failMode=standalone`,
   why the mount at `/workspace`. These are exactly the questions asked in the oral defense.
4. **Do not touch the supplied infrastructure or checks.** Files listed in
   `.github/policy/manifest.sha256` (including `Makefile` and `Dockerfile`) are integrity-checked.
   Official grading uses a fresh canonical bundle of `tests/` and policy files, not the student's
   local copies. Never "fix" a failing test by editing the test.
